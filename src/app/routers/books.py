...
@router.get("/books/", response_model=list[books.BookRetrieve])
def get_books(db: Session = Depends(get_db)):
    books = transactions.get_books(db)
    return books


@router.get("/books/{id}", response_model=books.BookRetrieve)
...
@router.post("/books/", response_model=books.BookRetrieve, status_code=201)
def create_book(book_data: books.Book, db: Session = Depends(get_db)):
    book = transactions.create_book(db, book_data)
    return book


@router.put("/books/{id}", response_model=books.BookRetrieve)
